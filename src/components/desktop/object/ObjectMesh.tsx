...
export function ObjectMesh({ id, shape, tint, symbol, artSrc, finish, dragging, carried, anyDragging, dimmed }: Props) {
  // refs
  ...
  useFrame((state, dt) => {
    ...
    const spinning = dragging || carried || hovered
    if (spinning) {
      spinRef.current.rotation.y += dt * SPIN_SPEED
    } else {
