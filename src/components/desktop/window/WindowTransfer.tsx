...
type Props = {
  ...
  to: PersonObj
  z: number
  /** A trade request opens straight on the Trade step with no seed assets — both sides fill it live. */
  initialStep?: Step
  onClose: () => void
...
export function WindowTransfer({ assets, inventory, to, z, initialStep = "choose", onClose, onSend, onLaunch }: Props) {
  // refs
  ...
  // a trade request carries no seed assets — the "what" is nothing until you add it at the table
  const what = !lead ? "" : assets.length > 1 ? `${assets.length}x assets` : editable ? `${units(amount)} ${single!.symbol}` : `${units(lead.balance)} ${lead.symbol}`
  ...
  // events
  // once you're at the trade table, closing means walking away — the negotiation (and the request
  // that opened it, if this came from one) can't be picked back up, so the backdrop and the X both
  // go through a confirm rather than closing outright
  const requestClose = () => (step === "handoff" ? setConfirmingClose(true) : onClose())
    ...
    <>
      <WindowShell
        ...
        <div ref={bodyRef} key={step} className="animate-in fade-in-0 p-28 duration-300">
          {/* with no seed assets there's no "choose" step to head back to — the request is trade-only */}
          {step !== "choose" && assets.length > 0 && (
            <button
              type="button"
      ...
      {/* the trade table can't be reopened once left — a stray backdrop click or a slipped X shouldn't
          be the thing that costs it, so both route through here instead of closing outright */}
      {confirmingClose && (
        <WindowShell z={z + 1} width={380} onClose={() => setConfirmingClose(false)}>
          <div className="p-28">
            <WindowHeading icon={<TriangleAlert className="size-20 text-warning" />} title="Cancel this trade?" />
            <p className="mt-24 text-13 leading-140 text-white/80">
              Closing now cancels the trade with {to.label} — once you leave the table there&apos;s no reopening it.
            </p>
            <div className="mt-28 flex gap-8">
              <BaseBtn variant="secondary" className="flex-1" onClick={() => setConfirmingClose(false)}>
                Keep negotiating
              </BaseBtn>
              <BaseBtn icon={X} className="flex-1 border-danger bg-danger text-white hover:bg-danger hover:brightness-110" onClick={onClose}>
                Cancel trade
              </BaseBtn>
            </div>
          </div>
